(() => {
  ...
  window.capabilityDetails = {
    ...
    'cmd-goal': {
      ...
      facts: [
        '五家 CLI 都提供 `/goal`；Qoder CLI 的 `/goal` 于 2026-08-23 在官方命令参考确认。',
      ...
      products: {
        ...
        qoder: command('qoder', ['/goal [description] [--turns <N>]', '/goal status', '/goal pause', '/goal resume', '/goal take', '/goal clear'], '创建或更新目标并持续工作直到完成；不带子命令等价于 `/goal status`，显示目标描述、状态、已用轮数（Elapsed Turns）与已用时间。`pause` 暂停、`resume` 恢复、`clear` 移除当前目标，`take` 认领其他会话持有的目标并把所有权转移到当前会话。', {
          parameters: '`<description>` 自然语言目标描述，省略时显示状态；`--turns <N>` 交互轮数上限，`N` 须为正整数，非法取值被忽略',
          persistence: '目标随会话保存（Goals are persisted with the session），状态字段含 `objective`、`status`、`maxTurns`、`turnsUsed`、`timeUsedSeconds`、`ownerSessionId`、`planWasActive`；进程在 active 状态意外退出后，下次启动自动降级为 paused',
          conditions: '目标状态为 `active`/`paused`/`complete`；`/goal` 从不改变权限模式（Goal never changes the Permission Mode），工具审批沿用当前权限模式，无人值守需事先切换到 `auto` 或 `yolo`；未设 `--turns` 时适用内置默认上限，`turnsUsed` 达到 `maxTurns` 时目标自动暂停，`/goal resume` 授予全新轮数预算继续；更新其他会话持有的目标必须先 `/goal take`，防止新窗口悄悄继承另一会话未完成的目标；目标激活前若处于 Plan 模式，`/goal resume` 在进程重启后同时恢复 Plan 模式；官方文档未列出非交互退出码',
          sources: ['qoder-commands', 'qoder-goal-reference'],
        }),
      },
    ...
    'cmd-tasks': {
      ...
      facts: [
        ...
        'Qwen Code `/tasks` 输出文本摘要，交互式任务对话框从底部状态入口打开；v0.21.8 起 Workflows 开启时，对话框内按 `p` 或用 `/workflows p <runId>` 可协作暂停/恢复后台 Workflow 运行。',
      ...
      products: {
        ...
        qwen: command('qwen', ['/tasks', '/workflows p <runId>'], '输出后台任务文本列表；交互式 Background Tasks 对话框中按 `p` 协作暂停或恢复选中的后台 Workflow 运行：暂停后不再启动新 Agent，已派发的 Agent 继续运行到收敛，Agent 调用之间的脚本代码继续执行，恢复后释放暂存结果并继续队列派发。`/workflows p <runId>` 在 TUI 提供同样的进程内控制，运行状态在 running、pausing、paused 之间迁移。', {
          parameters: '`/workflows p <runId>` 接收运行中的 Workflow runId；参数缺失报 `Usage: /workflows p <runId>`，runId 未知或状态不符各有错误提示',
          mode: '`/tasks` 支持交互式、非交互式、ACP；Workflow 暂停/恢复控制仅交互式 TUI，`-p` 与 ACP 报“Workflow pause controls are available only in the interactive TUI.”',
          persistence: '任务状态属于当前会话；Workflow 暂停状态只保留在当前进程，重启不保留',
          conditions: 'Workflow 暂停只作用于后台运行，前台 Workflow 运行报“Foreground workflow runs cannot be paused or resumed”；pausing 阶段不能撤销暂停，只能等待收敛或停止运行；`/workflows` 只在 `QWEN_CODE_ENABLE_WORKFLOWS=1` 启用 Workflows 时注册',
          sources: ['qwen-commands', 'qwen-workflow-pause-docs', 'qwen-workflow-pause-shortcuts', 'qwen-workflow-pause-source', 'qwen-v0218-release'],
        }),
      ...
      related: ['cmd-goal', 'agent-background', 'execution-background', 'cmd-custom'],
    },
    ...
    'cmd-collaboration': {
      ...
      facts: [
        ...
        'Qwen Code 的 `/advisor`（提交 `18c9763f46ce`，PR #7567）于 2026-08-17 合入、随 v0.21.14（2026-08-19 发布）发布：以工具全部移除的只读旁路查询请审查模型对当前对话给出二次意见，固定输出 Verdict、Risks、Missing evidence、Recommendation 四节，`advisorModel` 设置可把审查路由到其他模型（可跨 Provider）。',
    ...
    'cmd-keymap': {
      definition: '在终端交互界面中重新映射或自定义键盘快捷键，包括绑定文件、配置键、作用上下文和连续按键组合。',
      includes: ['快捷键重映射入口', '绑定文件与配置键', '上下文与动作格式', '连续按键 chord'],
      excludes: ['Vim 模式切换本身', '固定内置快捷键的完整清单', 'IDE 或桌面端快捷键'],
      facts: [
        'Claude Code 与 Codex 提供逐项快捷键自定义；Qwen Code、Kimi Code 与 Qoder CLI 当前官方文档未列出同类入口。',
        'Claude Code 通过 `~/.claude/keybindings.json` 绑定，支持空格分隔的 chord（如 `ctrl+k ctrl+s`），修改后自动生效。',
        'Codex 在 2026-08-01 的官方仓库提交中为 `tui.keymap` 增加双键 chord（如 `ctrl-x ctrl-s`）支持，官方配置文档尚未记录 chord 语法。',
      ],
      products: {
        claude: command('claude', ['/keybindings'], '打开或创建 `~/.claude/keybindings.json`；按上下文绑定、改绑快捷键，或把动作设为 `null` 解绑默认快捷键。', {
          parameters: '绑定文件为 `bindings` 数组，每项含 `context` 与 `namespace:action` 动作映射',
          persistence: '写入 `~/.claude/keybindings.json`；修改自动检测生效，无需重启',
          conditions: '上下文包括 `Global`、`Chat`、`Autocomplete` 等；chord 用空格分隔连续按键；`Ctrl+C`、`Ctrl+D`、`Ctrl+M` 与 Caps Lock 保留不可重绑；Vim 模式按键不经此文件重映射，需用 `vimInsertModeRemaps`',
          sources: ['claude-commands', 'claude-keybindings'],
        }),
        codex: command('codex', ['/keymap'], '重映射 TUI 快捷键并持久化到 `config.toml`；可捕获、添加、替换和查看绑定，包括双键 chord。', {
          parameters: '`tui.keymap.<context>.<action>`；context 包括 `global`、`chat`、`composer`、`editor`、`vim_normal`、`pager`、`list`、`approval` 等；空数组解绑',
          persistence: '写入 `config.toml`',
          conditions: '双键 chord（如 `ctrl-x ctrl-s`）最多两键；待命中 chord 在页脚提示，`esc`、上下文切换或 1 秒超时后取消；与已启用单键绑定、跨上下文 chord 或保留终端快捷键冲突时拒绝；chord 行为来自官方仓库提交，官方配置文档尚未记录',
          sources: ['codex-commands', 'codex-config-reference', 'codex-keymap-chords'],
        }),
        qwen: {
          commands: [],
          aliases: [],
          parameters: '—',
          behavior: '官方设置与命令文档未列出逐项快捷键自定义；只提供 `/vim` 开关与 `general.vimMode`（默认 `false`）启用 Vim 按键。',
          mode: '交互式 CLI',
          persistence: '—',
          conditions: '固定快捷键以官方文档内说明为准；不据此推断底层能力不存在',
          status: '未确认',
          sources: ['qwen-session-settings', 'qwen-commands-current'],
        },
        kimi: {
          commands: [],
          aliases: [],
          parameters: '—',
          behavior: '官方命令目录与配置文档未列出快捷键自定义入口；`tui.toml` 仅覆盖主题、粘贴、通知等界面偏好。',
          mode: '—',
          persistence: '—',
          conditions: '固定快捷键以官方文档内说明为准；不据此推断底层能力不存在',
          status: '未确认',
          sources: ['kimi-commands-current', 'kimi-config-current'],
        },
        qoder: {
          commands: [],
          aliases: [],
          parameters: '—',
          behavior: '官方文档未列出逐项快捷键自定义；只提供 `/vim` 切换 Vim 编辑模式。',
          mode: 'TUI',
          persistence: '—',
          conditions: '固定快捷键以官方文档内说明为准；不据此推断底层能力不存在',
          status: '未确认',
          sources: ['qoder-commands', 'qoder-using-cli'],
        },
      },
      related: ['cmd-config', 'cmd-hooks', 'cmd-status'],
    },
  };
})();
