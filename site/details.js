  ...
  window.capabilityDetails = {
    ...
    'cmd-collaboration': {
      ...
      facts: [
        ...
        'Kimi Code 于 2026-08-16 合入 `/tower`（提交 `f492cd7c9e03`，PR #2633），2026-08-18 又在 main 分支整体禁用（提交 `5ae82cd5bcb9`，PR #3023）：Tower Skill 被标记未注册的 `tower` 实验标志，Skill、十一个 `Tower*` 工具与 `tower-worker` profile 均不加载，环境变量、master 开关或 `[experimental]` 配置都无法开启，changeset 已删除；禁用前的设计为主 Agent 作为唯一控制塔规划任务并合并分支，worker 在各自 git worktree 中执行任务，reviewer 审查分支，合并由代码级门禁强制（需针对当前分支 tip 的 clean review）。',
      ],
      products: {
        ...
        kimi: command('kimi', ['/swarm on|off', '/swarm <task>', '/tower'], '切换 Swarm 模式，或为单轮任务开启 Swarm 并在成功完成后自动关闭。`/tower` 曾于 2026-08-16 合入 main（提交 `f492cd7c9e03`，PR #2633），2026-08-18 提交 `5ae82cd5bcb9`（PR #3023）将 tower 功能整体禁用：Tower Skill 被标记 `experimentalFlag: \'tower\'`，CLI 与 kap-server 的内置 Skill 加载源都会过滤未启用标志的 Skill，`/tower` 不再出现在可用命令中。禁用前的设计为在 v2 引擎进入 Tower 模式：主 Agent 作为唯一控制塔，只做任务规划（`TowerPlan`）、派生 worker/reviewer（`TowerSpawn`）、路由信息与合并分支，不写产品代码；每个 worker 在 `.tower/worktrees/` 下的独立 git worktree 中执行一个 mission，reviewer 审查分支并经 `TowerReview` 提交结论；inbox、findings、reviews、mission 文件与 activity.log 全部由十一个 `Tower*` 工具写入 `.tower/comms/`，禁止手工编辑 `.tower/` 文件。', {
          parameters: '`/swarm` 使用 `on|off|<task>`；`/tower` 不带参数或带 `status` 查看状态，带 `teardown` 拆除工作区，其余输入作为目标',
          mode: '交互式 TUI；`/tower`（当前禁用）为 v2 引擎内置 Skill（`disable-model-invocation: true`），只能由用户显式调用；2026-08-16 同一提交起 Skill 命令在回合运行时输入会排队而不是被拒绝，Ctrl-S 可作为 activation 注入运行中的回合',
          persistence: 'Swarm 模式属于当前会话；`/tower`（当前禁用）在原设计中于当前仓库目录创建 `.tower/` 工作区：`state.json` 记录基线分支与 Agent 名册，worktree 位于 `.tower/worktrees/`，teardown 后 `.tower/comms/`（含活动日志）保留作为审计记录；`tower_mode.enter`/`tower_mode.exit` 写入会话 wire 历史，恢复会话可继续；合并后分支保留',
          conditions: '`/tower` 于 2026-08-16 合入 main（提交 `f492cd7c9e03`，PR #2633），从未进入 Release；2026-08-18 提交 `5ae82cd5bcb9`（PR #3023）整体禁用：`TowerFeature` 构造函数在 `tower` 标志未启用时直接返回，不贡献十一个 `Tower*` 工具、`tower-worker` profile 与限流服务，`AgentTowerService.enter()` 与工具守护也先检查该标志；`tower` 标志 id 未注册进实验功能登记册，`enabled(\'tower\')` 恒为 false，官方测试固定了该行为：`KIMI_CODE_EXPERIMENTAL_TOWER=true`、master 开关 `KIMI_CODE_EXPERIMENTAL_FLAG=true` 或 `[experimental] tower = true` 都无法启用，`/experiments` 面板不会列出该标志；同一 PR 删除了 changeset `.changeset/tower-slash-command.md`；官方 Slash 命令文档未列出 `/tower`；仅 v2 引擎实现（agent-core-v2）；以下为禁用前的设计边界：`TowerInit` 要求至少有一个提交的 git 仓库；worker/reviewer 派生时固定 `auto` 权限模式，会话整体切换模式不影响它们；worker 在 secondary-model 实验开启时绑定 `[secondary_model]` 模型池，否则继承塔模型，reviewer 始终绑定主力模型；`TowerMerge` 门禁在最新 review 非 clean、分支在 review 后移动、依赖 mission 未合并、改动文件超出 mission 作用域 glob 或主检出不在记录的基线分支时拒绝合并；Tower 模式下 `TodoList` 工具被代码禁用，worker 无 `AskUserQuestion`；派生并发受预算限制（上限 16，遇 429 暂停 60 秒）；manual 权限模式下启动 `/swarm` 任务会询问是否切换到 auto 或 yolo',
          status: '源码确认',
          sources: ['kimi-commands', 'kimi-tower-commit', 'kimi-tower-changeset', 'kimi-tower-skill', 'kimi-tower-spawn', 'kimi-tower-disable-commit', 'kimi-tower-skill-gate', 'kimi-tower-feature-source', 'kimi-tower-flag-test', 'kimi-builtin-skill-filter'],
        }),
