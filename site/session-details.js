(() => {
  ...
  window.capabilityDetails = Object.assign(window.capabilityDetails ?? {}, {
    'session-resume': createDetail({
      ...
      related: ['session-branch', 'session-naming', 'session-checkpoint'],
    }),
    ...
    'session-branch': createDetail({
      ...
      products: {
        ...
        qoder: {
          ...
          conditions:
            '只有与 `resume` 组合时构成会话分支；不能据此推断 TUI 存在同名命令。',
    ...
    'session-messaging': createDetail({
      ...
      facts: [
        '只有 Claude Code 提供独立会话之间的消息：`ListAgents`/`/list-agents` 发现本地会话、Subagent 与 Remote Control 会话，`SendMessage` 按名称投递；v2.1.224 引入，v2.1.225 支持按名称主动发起对其他机器 Remote Control 会话的对话，v2.1.229 为列表增加 `offline`/`cloud` 状态标签，v2.1.232 增加提示词 `@` 会话名提及、`SendMessage` 裸名投递与同机唯一会话名。',
