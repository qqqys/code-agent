  ...
  window.capabilityDetails = Object.assign(window.capabilityDetails ?? {}, {
    ...
    'surface-web': createDetail({
      ...
      facts: [
        ...
        'Qwen Web Shell 自 v0.21.9 支持图片拖拽/粘贴输入与图片-only prompt，自 v0.21.12-preview.3 预览通道支持把本地文件直传进工作区；Claude、Codex、Kimi、Qoder 的官方 Web 文档未列浏览器侧图片拖拽、粘贴或本地文件上传（Claude 可由 CLI `claude --cloud` 把整个本地仓库打包上传到云会话，不属于浏览器侧文件上传）。',
      ],
      products: {
        claude: {
          entry:
        ...
        qwen: {
          entry:
            '`qwen serve` 根路径自带 Web Shell；`--open` 自动打开浏览器，`--no-web` 可禁用；条件：图片输入经输入区拖拽或粘贴进入（v0.21.9 起）；工作区文件上传经输入区拖入普通文件或 `@` 文件面板的“上传文件”进入（v0.21.12-preview.3 预览通道）。',
          protocol:
            '同源静态 Web App 通过 HTTP REST 与 SSE 连接本地 daemon；图片以 base64 附件随 prompt 载荷提交；文件上传走独立的二进制路由 `POST /file/upload`（主工作区）与 `POST /workspaces/:workspace/file/upload`（qualified 工作区），要求 `Content-Type: application/octet-stream`（否则 415），不复用文本写入路由；v1 无 ACP-HTTP 对等上传方法。',
          behavior:
          ...
          state:
            '会话由本地 daemon 和磁盘 transcript 管理；多浏览器客户端可共享同一会话；条件：排队 prompt 的图片附件在重试/编辑流程中恢复；admission 结果未知时输入区进入只读锁定，需手动丢弃或恢复，恢复不自动发送；页面重载后恢复的排队项仅含摘要、不含图片数据。上传进度显示在输入区上方，成功条目三秒后消失，结果成为可移除的行内文件标签（使用服务端确认的路径）；TypeScript SDK 的 `uploadWorkspaceFile`（`DaemonClient` 与 `WorkspaceDaemonClient`）提供同一上传操作，支持进度回调、取消与超时。',
          tools:
