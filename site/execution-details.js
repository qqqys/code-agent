  ...
  window.capabilityDetails = Object.assign(window.capabilityDetails ?? {}, {
    ...
    'execution-review': createDetail({
      ...
      facts: [
        ...
        'Qwen Code v0.21.15 起 `/review` 与 `qwen review run` 支持 `--resume`：中断的 PR 审查从 worktree、diff 与 Agent 运行记录等磁盘状态继续而不是重新开始，续跑复用上一次尝试已认证的 Agent 结果；续跑固定在被中断运行的 effort，PR head 移动或 effort 不一致时拒绝续跑并回退为全新审查；其余四家当前一手资料未列出同类恢复中断 Review 的内置入口。',
      ],
      products: {
        claude: {
          entry:
        ...
        qwen: {
          entry:
            '`/review` 审本地变化；`/review <file>` 审文件；`/review <pr>` 审 PR；`--comment` 发布 GitHub Review；`/review <pr> --resume` 继续该 PR 上一次被中断的审查（v0.21.15 起）；Headless 包装 `qwen review run [target] [--json] [--fail-on request-changes] [--comment] [--resume] [--quiet]` 同样支持 `--resume`；`qwen review publish-assets` 发布证据图；`qwen review cost-ledger --plan <计划报告> [--out <路径>]` 聚合本次审查成本；`qwen review repo-context --plan <计划 JSON> --worktree <工作树> --out <产物路径>` 为审查计划附加有界仓库上下文，medium/high effort 审查在计划采集后由流程调用。',
          primitives:
          ...
          behavior:
            '本地默认 medium effort，PR 默认 high；同仓 PR 强制在临时 Worktree 中读、测、审，跨仓 URL 使用轻量模式。证据图只接受 png/jpg/jpeg/gif/webp（拒绝 SVG），单文件 10 MiB、单批 40 MiB 上限，任一文件不合格则整批拒绝。medium/high effort 审查还会把规范 findings 与组合结论保存为与报告同名的结构化 JSON 产物，Web Shell 把它渲染成可筛选 findings 的交互式审查视图，Markdown 报告保持人类可读存档。`cost-ledger` 以 `--plan` 计划报告的 mtime 为计费窗口起点（计划前的引导轮次与台账运行后的组装不计入），按流输出主循环与每个审查 Agent 的模型调用数、输入/缓存/输出/思考 token 数与耗时；模型调用只统计携带 `usageMetadata` 的 assistant 记录，计数是下限而非精确 API 调用数。仓库上下文规则在任一变更文件命中其 `paths` glob 时生效，所有命中规则的指引合并、去重并排序：审查 Agent 获得领域与相关文件，build-and-test Agent 获得推荐测试、必需配置与验证说明，`requiredAgents` 只在所选 effort 与拓扑本就运行相应角色时并入名册，`unverifiedDimensions` 在最终审查中作为非阻塞证据边界披露。`relatedPaths` 通配符从工作树展开（启用点文件、不返回目录、不跟随符号链接），必须以非通配目录段开头，解析文件必须留在工作树内且不进入 `node_modules`、`dist` 等依赖或构建产物目录；访问条目 16384、解析文件 128 或匹配工作量预算任一超限即失败关闭。`repo-context` 校验 `--out` 与 `--plan` 不是同一文件、worktree 是目录且与 `plan.worktreePath` 一致、`plan.mergeBaseSha` 格式有效且可解析，成功后把上下文 JSON 写入 `--out` 并原子覆写计划文件，stdout 输出 `Wrote repository context (manifest) to <路径>`，无上下文时输出 `Wrote null repository context to <路径>`。`--resume` 续跑复用上一次尝试的 worktree、计划、diff 与报告：`--out` 路径上的旧报告刻意保留不改动，只用来读取 worktree、计划和 diff。续跑固定在被中断运行记录的 effort，显式传不同 `--effort` 会以 `effort-mismatch` 拒绝续跑并按新级别全新运行；PR head 已移动时以 `head-moved` 拒绝，该拒绝本身消耗本次审查的一次 head 移动重启配额，之后 Step 7 只能在已审 SHA 提交、不得再次重启。续跑从最后一次已认证的反向审计轮次 `k` 之后以 `k+1` 继续，续跑披露自动化：覆盖统计计入 `recoveredAgents`，组合正文自带续跑说明行。',
          scope:
            '本地 working tree、单文件、PR number 或 URL；规则来自系统、项目 AGENTS.md 和 Skill。`cost-ledger` 的记录位置来自 CLI 导出的 `QWEN_CODE_PROJECT_DIR`/`QWEN_CODE_SESSION_ID` 环境变量，不接受模型指定的路径。仓库上下文只在 medium/high effort 的本地与同仓 PR 审查中生效；low effort 与跨仓轻量审查跳过。PR 审查的清单从 merge base 提交读取，PR head 无法为自己加入或移除指引；本地审查的清单从当前 worktree 读取。`--resume` 只对 PR 目标有效：本地审查的 diff 来自活动工作树，没有可续跑的稳定中断状态，非 PR 目标传 `--resume` 会收到警告并被忽略；跨仓且无匹配 remote 的 PR URL 走轻量模式，没有 `fetch-pr`、worktree 和计划，`--resume` 同样无效、审查从头运行。',
          background:
            '多维度审查 Agent 可并行执行；进度在当前 Surface 展示。',
          integration:
            '`--comment` 在 GitHub 提交一次 Review；Qwen Code Action 还能在 PR 事件中自动运行审查。GitHub API 不能给 Review 评论附图，`publish-assets` 把证据图托管到 `QWEN_REVIEW_ASSETS_REPO`（`owner/repo`），写入 `pr-assets/<pr>-review` 分支并以 commit 固定的 URL 嵌入评论；GitHub Enterprise 加 `--host`。续跑定位为本地便利：仓库自身的 CI 审查工作流不续跑，每次重试都全新运行（临时沙箱退出即删除 worktree，没有可恢复的磁盘状态，场景测试固定该缺失）。',
          artifacts:
