  ...
  window.capabilityDetails = Object.assign(window.capabilityDetails ?? {}, {
    ...
    'execution-review': createDetail({
      ...
      products: {
        ...
        qwen: {
          entry:
            '`/review` 审本地变化；`/review <file>` 审文件；`/review <pr>` 审 PR；`--comment` 发布 GitHub Review；`qwen review publish-assets` 发布证据图。',
          primitives:
            '内置 Skill 用 `qwen review fetch-pr`、并行审查 Agent、锚点验证和一次 Create Review API 提交；证据图经 Contents API（`gh` HTTPS）写入指定仓库，不克隆、不走 SSH。',
          behavior:
            '本地默认 medium effort，PR 默认 high；同仓 PR 强制在临时 Worktree 中读、测、审，跨仓 URL 使用轻量模式。证据图只接受 png/jpg/jpeg/gif/webp（拒绝 SVG），单文件 10 MiB、单批 40 MiB 上限，任一文件不合格则整批拒绝。medium/high effort 审查还会把规范 findings 与组合结论保存为与报告同名的结构化 JSON 产物，Web Shell 把它渲染成可筛选 findings 的交互式审查视图，Markdown 报告保持人类可读存档。',
          scope:
            '本地 working tree、单文件、PR number 或 URL；规则来自系统、项目 AGENTS.md 和 Skill。',
          background:
          ...
          artifacts:
            '同仓审查在项目 `.qwen/reviews/` 保存 Markdown 报告，medium/high effort 另有同名结构化 JSON 产物（跨仓轻量审查不落盘报告）；本地 findings、可选 GitHub 行内评论；findings 带 `assetFiles` 本地路径与 `assets` 已发布 URL，资产清单记录每个文件及落点 commit；Worktree 按流程清理。',
          conditions:
            'Bare mode、禁用 Skills 或 Slash 时不可用；PR 读取/评论需要 GitHub 访问权限。`publish-assets` 还要求 `QWEN_REVIEW_ASSETS_REPO` 指定可推送仓库（未设置或格式错误退出码 3，不自动选仓库），并与 `submit` 共用授权门禁：只有被授权发布评论的运行才能推送，有效 `--comment` 强制 high effort，因此 low/medium 运行不会发布。结构化 JSON 产物与 Web Shell 视图只来自 medium/high effort 审查；low effort 不生成。',
          sources: [
            'qwen-review-current',
            'qwen-review-skill',
            'qwen-review-assets',
    ...
    'execution-computer-use': createDetail({
      ...
      facts: [
        'Qwen Code 在 CLI 内默认注册 `computer_use__*` 桌面控制工具，并含浏览器 `page` 工具；Kimi Code 于 2026-08-04 把 `kimi-cu` 与 `kimi-webbridge` 作为 `/plugins` 内置能力合入主分支，尚未进入发布版本。',
      ...
      products: {
        ...
        kimi: {
          entry:
            'v2 CLI 的 `/plugins` 面板内置 `Kimi Computer Use`（`kimi-cu`）与 `Kimi WebBridge`（`kimi-webbridge`）条目，安装时一并配置最新托管运行时与插件，报告缺失的手动步骤并支持中断后重试。',
          primitives:
            '`kimi-cu` 安装 `KimiCU.app`、同名插件与 `ai.kimi.cu.service` launchd 服务，提供后台 macOS GUI 自动化：读取应用 UI、点击、输入、滚动、拖拽，不抢占鼠标或前台；`kimi-webbridge` 安装 `~/.kimi-webbridge/bin/` 守护进程与官方浏览器控制插件，控制带登录态的真实浏览器（导航、点击、输入、读取页面、截图），浏览器扩展为可选手动安装。',
          behavior:
            '安装为幂等托管流程：按插件、应用、服务、权限（`kimi-cu`）或守护进程、插件、扩展连接（`kimi-webbridge`）逐项检测就绪状态；WebBridge 守护进程只在未运行时启动（start-if-down，与 Kimi Work 共存）。',
          scope:
            '`kimi-cu` 仅 macOS；`kimi-webbridge` 支持 macOS arm64/x64、Linux arm64/x64、Windows x64；条目由客户端按发布版本注入默认 Official marketplace，旧版本客户端不会看到。',
          background:
            '`kimi-cu` 经后台 launchd 服务运行；`kimi-webbridge` 守护进程常驻，就绪检测轮询 `/status`。',
          integration:
            'macOS 辅助功能与屏幕录制（TCC）权限只能由用户手动授予，就绪检测经 `xpc-ping` 校验；WebBridge 会检测 `~/.kimi-code/skills/` 与 `~/.agents/skills/` 下冲突的旧技能副本并在安装进度中标记迁移。',
          artifacts:
            '`KimiCU.app`、launchd 服务、`~/.kimi-webbridge/bin/kimi-webbridge[.exe]` 及版本文件、已安装的官方插件。',
          conditions:
            '2026-08-04 合入主分支，changeset 标记为 minor，当前发布版本（0.32.0）尚未包含；仅 v2 CLI 提供；安装进行中会返回 `capability.install_in_progress`。',
          status: '条件项',
          sources: ['kimi-builtin-capabilities'],
        },
        qoder: {
          entry:
