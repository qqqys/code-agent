...
| [代码 Review](./capabilities/execution/execution-review.md) | `/review` 为 `/code-review` 别名 · `ultra` 云审查 · GitHub Review | `/review` · GitHub Review | `/review` 内置 Skill · `publish-assets` 证据图 · Web Shell 结构化结果 · `cost-ledger` 成本台账 | 自然语言；无内置 `/review` | `/review [instruction]` |
