...
| Kimi Code | `/swarm on\|off`、`/swarm <task>`、`/tower` | 源码确认 |
...
6. Qwen Code 于 2026-08-17 合入 `/advisor`（提交 `18c9763f46ce`，PR #7567，合入 main 尚未发布）：以工具全部移除的只读旁路查询请审查模型对当前对话给出二次意见，固定输出 Verdict、Risks、Missing evidence、Recommendation 四节，`advisorModel` 设置可把审查路由到其他模型（可跨 Provider）。
