...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-memory)
...
1. Claude Code 与 Qoder CLI 明确提供 Agent `memory` 字段。
2. Codex、Qwen Code 与 Kimi Code 当前 Agent 字段表未确认等价的独立持久记忆字段。
...
| 具体行为 | `memory` 可选 `user`、`project`、`local`，为该 Agent 建立跨会话目录。 |
...
| 具体行为 | Codex 产品有 Memories，但当前 Subagent Agent 文件 schema 未列出 Agent 独立 memory 字段。 |
...
| 具体行为 | Qwen Code 有产品级记忆能力；当前 Subagent frontmatter 的 `memory` 尚未落地。 |
...
| 具体行为 | 会话可保留 Agent 运行状态，但当前 Agent 文件字段未列出跨任务持久 `memory`。 |
...
| 具体行为 | `memory` 可选 `user`、`project`、`local`；只有启用全局自动记忆时才生效。 |
