...
| Codex | `model_reasoning_effort` | 官方确认 |
...
2. Qwen Code 和 Kimi Code 当前 Agent 文档未确认独立 effort 字段。
...
| 具体行为 | `effort` 覆盖会话 effort，可用档位取决于模型；扩展思考开关仍继承主会话。 |
...
| 具体行为 | 当前 `effort` frontmatter 尚未落地；模型 grade 和 `model` 选择不等同于推理强度。 |
...
| 具体行为 | `model_preference` 选择主/备模型，不是独立 reasoning effort；当前字段表未列出 effort。 |
...
| 具体行为 | `effort` 接受 `low`、`medium`、`high`、`xhigh`、`max` 或正整数预算。 |
