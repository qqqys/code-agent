...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-hooks)
...
| 具体行为 | `hooks` 可定义该 Subagent 的生命周期 Hook；插件 Agent 中该字段被忽略。 |
...
| 具体行为 | `hooks` 作用于 Subagent 会话，支持工具、启动、停止、通知等事件和多种 Hook 类型。 |
