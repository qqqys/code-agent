...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-deny-tools)
...
| 产品 | 结论 | 证据状态 |
| --- | --- | --- |
| Claude Code | `disallowedTools` | 官方确认 |
| Codex | 未确认独立字段 | 未确认 |
| Qwen Code | `disallowedTools` | 源码确认 |
| Kimi Code | `disallowedTools` | 官方确认 |
| Qoder CLI | `disallowedTools` | 官方确认 |
...
1. Claude Code、Qwen Code、Kimi Code 与 Qoder CLI 都提供 `disallowedTools`。
2. Codex 当前 Agent 文件 schema 未列出对应 denylist 字段。
...
| 具体行为 | `disallowedTools` 删除继承或指定工具；与 `tools` 同时出现时，被禁止的工具不会恢复。 |
...
| 具体行为 | 当前 Subagent Agent 文件字段未列出 `disallowedTools`；可通过只读沙箱等配置限制行为。 |
...
| 具体行为 | `disallowedTools` 在 `tools` allowlist 之后删除工具，并支持 MCP Server 级模式。 |
...
| 具体行为 | `disallowedTools` 在 `tools` 之后应用；工具名精确匹配，MCP 使用 glob。 |
...
| 具体行为 | `disallowedTools` 在工具注册后应用；禁止 `Agent` 可完全停止继续派发。 |
