...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-context)
...
1. 五家命名 Subagent 都把中间搜索和工具输出放在独立上下文中。
2. Qwen Code 另有 Fork：它仍是独立运行，但起点可继承父会话历史。
...
| 具体行为 | 每个 Subagent 有独立上下文窗口；中间输出不进入主会话，只返回最后结果。 |
...
| 具体行为 | 每个 Subagent 使用独立线程；父线程能打开、跟进和汇总，但不会把全部子线程日志并入主历史。 |
...
| 具体行为 | 命名 Agent 有独立历史；Fork 复制父上下文前缀后在独立后台运行。 |
...
| 具体行为 | 子 Agent 在自己的上下文中工作，中间思考和工具记录不混入主 Agent 历史。 |
...
| 具体行为 | Subagent 有独立上下文、Transcript 与压缩流程，中间搜索和推理不直接进入主会话。 |
