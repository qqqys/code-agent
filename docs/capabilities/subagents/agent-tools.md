...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-tools)
...
| 具体行为 | `tools` 支持 YAML 列表或逗号字符串；省略或 `*` 表示全部，空列表表示禁用全部。 |
...
| 具体行为 | `tools` 支持字符串或数组，并可用 `Agent(Explore, Plan)` 只允许指定下级 Agent。 |
