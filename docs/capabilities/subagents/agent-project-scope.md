...
| Kimi Code | `.kimi-code/agents/` · `.agents/agents/` | 官方确认 |
...
1. 五家都提供仓库内的项目级 Agent 目录。
2. Kimi Code 同时扫描产品专属目录和通用 `.agents/agents/` 目录。
...
| 具体行为 | 从当前目录向仓库根扫描 `.claude/agents/`，嵌套目录中离工作目录更近的同名定义优先。 |
...
| 具体行为 | 项目 Agent 放在 `.codex/agents/*.toml`，只对当前项目生效。 |
...
| 具体行为 | 项目 Agent 放在 `.qwen/agents/*.md`，优先于用户和扩展 Agent。 |
...
| 具体行为 | 项目 Agent 放在 `.qoder/agents/*.md`，可纳入版本控制并依赖文件夹信任。 |
