...
[返回 Subagent 详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=agent-user-scope)
...
| 产品 | 结论 | 证据状态 |
| --- | --- | --- |
| Claude Code | `~/.claude/agents/` | 官方确认 |
| Codex | `~/.codex/agents/` | 官方确认 |
| Qwen Code | `~/.qwen/agents/` | 源码确认 |
| Kimi Code | `$KIMI_CODE_HOME/agents/` · `~/.agents/agents/` | 官方确认 |
| Qoder CLI | `~/.qoder/agents/` | 官方确认 |
...
1. 五家都提供用户级 Agent 定义目录。
2. 用户级定义通常低于项目级定义；具体同名优先级仍以各产品规则为准。
...
| 具体行为 | `~/.claude/agents/` 递归扫描，面向当前用户所有项目；项目定义的优先级更高。 |
...
| 具体行为 | `~/.codex/agents/*.toml` 为个人 Agent，项目同名 Agent 可覆盖它。 |
...
| 具体行为 | `~/.qwen/agents/*.md` 为用户级回退来源，低于项目 Agent。 |
...
| 字段 | 记录 |
| --- | --- |
| 矩阵结论 | `$KIMI_CODE_HOME/agents/` · `~/.agents/agents/` |
| 入口与配置 | 主 Agent 依据描述自动派发，也可在提示词中点名；`--agent-file` 可在启动时显式加载定义。 |
| 定义格式 | Markdown 正文 + YAML frontmatter；正文作为 Agent 系统提示词模板。 |
| 具体行为 | 扫描 `$KIMI_CODE_HOME/agents/` 与 `~/.agents/agents/`；产品目录可随 `KIMI_CODE_HOME` 移动。 |
...
| 具体行为 | `~/.qoder/agents/*.md` 对当前用户的所有项目生效。 |
