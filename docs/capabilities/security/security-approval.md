...
[返回权限与沙箱详情目录](./README.md) · [打开网页详情](https://qqqys.github.io/code-agent/capability.html?id=security-approval)
...
1. 五家都具备交互审批，但参与决策的工具范围、规则优先级和宿主回调不同。
2. Claude Code、Qwen Code、Kimi Code 与 Qoder CLI 都公开了工具级规则；Codex 把审批策略与沙箱边界分开配置。
...
| 具体行为 | 默认模式下，文件修改和需要授权的 Bash 调用弹出确认；规则可直接允许、强制询问或拒绝。 |
...
| 具体行为 | `on-request` 允许 Agent 在越出沙箱、请求网络或调用需确认工具时发起审批；可由用户或 Auto-review 处理符合条件的请求。 |
...
| 具体行为 | Ask Permissions 模式要求文件编辑和 Shell 命令确认；Auto 模式由分类器给出允许或阻断，显式 deny 仍优先。 |
...
| 具体行为 | 副作用工具弹出审批面板，可单次允许、拒绝或对同类调用放行当前会话；永久规则写入配置文件。 |
...
| 具体行为 | 每次工具调用得到 allow、ask 或 deny；TUI 消费 ask 为确认面板，SDK 与 ACP 可转给外部宿主。 |
...
- [权限设置](../commands/cmd-permissions.md)
